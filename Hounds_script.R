...
Clean_String <- function(string){
  # Lowercase
  string<-iconv(string, "UTF-8", "UTF-8",sub=' ')
...
#sentence <- "carry\x97dignified, solid, and reassuring."
...
con <- file("HoundBaskervilles.txt", "r", blocking = FALSE)
text <- readLines(con)

close(con) 

clean_speech <- Clean_Text_Block(text)
print(str(clean_speech))
